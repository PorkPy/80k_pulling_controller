...
def goal_pos_calc(angle = 0, distance = 0):
	...
	# If called empty: finds the x and y coordinates of the goal position using trig.
	# If called full: finds x,y of new iterative step.
	angle = angle * np.pi/180
	x = distance*np.sin(angle) # opposite side x.
	y = distance*np.cos(angle) # adjacent side y. 6/arcsin = x
	...
	y = np.sqrt(y**2)*-1 # This asserts that y is negative.
	#y = y*(-1)
